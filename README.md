...
- определение "чужих" изображений – если уверенность ниже порога, сервис честно
  сообщает, что это не похоже ни на один из 36 классов, вместо навязывания ответа;
...
- переключатель светлой/тёмной темы (запоминается в браузере, по умолчанию – системная тема);
- интерфейс на трёх языках – русский, қазақша, English (vue-i18n, запоминается в браузере);
- страница "О проекте" с метриками, пайплайном обучения и стеком технологий;
- rate limiting на тяжёлых по CPU эндпоинтах, автотесты бэкенда и фронтенда, CI/CD.
